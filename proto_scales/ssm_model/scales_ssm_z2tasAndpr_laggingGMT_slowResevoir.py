...
class DeepSSMPatternConditioned(nn.Module):
    ...
    def __init__(self, y_dim, u_dim, z_dim=16, rnn_hidden=62, u_rnn_hidden=64, mlp_hidden=128,
                 ...
                 init_alpha=0.01, init_omega=1.0, alpha_max=0.02):
        super().__init__()
        self.y_dim = y_dim
        self.u_dim = u_dim
        self.z_dim = z_dim
        ...
        if emission_uses_u:
            ...
            self.omega = nn.Parameter(torch.full((y_dim, reservoir_dim), init_omega))
    ...
    def _reservoir_step(self, s, u_t):
        """
        s:   [B, y_dim, reservoir_dim]
        u_t: [B, u_dim]  — treated as scalar forcing (mean over u_dim)
        returns s_next: [B, y_dim, reservoir_dim]
        s_{t+1,r} = s_{t,r} + alpha_r * (omega_r * u_t - s_{t,r})
        """
        alpha = torch.sigmoid(self.log_alpha) * self.alpha_max  # [y_dim, reservoir_dim], in (0, alpha_max)
        u_s = u_t.mean(dim=-1)                         # [B]
        target = self.omega[None] * u_s[:, None, None] # [B, y_dim, reservoir_dim]
