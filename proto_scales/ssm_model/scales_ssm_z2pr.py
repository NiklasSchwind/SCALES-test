...
class DeepSSMPatternConditioned(nn.Module):
    ...
    def __init__(self, y_dim, u_dim, z_dim=16, rnn_hidden=62, mlp_hidden=128, emission_uses_u=False):
        ...
        self.emit_pr = scales_ssm.MLP(emit_in,y_dim,hidden = mlp_hidden)
    ...
    def forward_elbo(self, y, pr,u, kl_free_bits=0.5):
        ...
        for t in range(T):
            ...
            if self.emission_uses_u:
                ...
                pr_hat = self.emit_pr(torch.cat([z_t, u[:, t]], dim=-1))
            else:
                res = self.emit(z_t)  # [B, y_dim]  (residual from latent)
                pr_hat = self.emit_pr(z_t)
            ...
            nll = nll + nll_t
            nll_t_pr = 0.5 * (((pr[:, t] - pr_hat) / sigma_pr) ** 2).sum(-1) + self.y_dim * torch.log(sigma_pr)
            nll_pr = nll_pr + nll_t_pr
        ...
        for _ in range(n_samples):
            ...
            for k in range(steps):
                ...
                if self.emission_uses_u:
                    ...
                    res_pr = self.emit_pr(torch.cat([z, u_t], dim=-1))
                else:
                    res = self.emit(z)
                    res_pr = self.emit_pr(z)
                
                y_hat = ctrl + res

                y_s = y_hat + sigma_y * torch.randn_like(y_hat)
                y_s_pr = res_pr + sigma_pr * torch.randn_like(res_pr)
                preds.append(y_s)
