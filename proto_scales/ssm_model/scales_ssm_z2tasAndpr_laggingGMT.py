...
class DeepSSMPatternConditioned(nn.Module):
    ...
    """
    def __init__(self, y_dim, u_dim, z_dim=16, rnn_hidden=62, u_rnn_hidden=64, mlp_hidden=128, emission_uses_u=False,use_linear_model=True):
        super().__init__()
        self.y_dim = y_dim
        self.u_dim = u_dim
        self.z_dim = z_dim
        ...
        emit_in = z_dim + (u_rnn_hidden if emission_uses_u else 0)
...
def run_train(
    ...
    raw_model = DeepSSMPatternConditioned(y_dim=Dy, u_dim=Du, z_dim=z_dim,rnn_hidden=rnn_hidden,
        use_linear_model=use_linear_model,emission_uses_u=True).to(device)
