...
monthly_flag = True
use_smoothing = False
TRAIN_SCENARIOS = [ 'ssp585','1pctco2']#,'ssp245']#['ssp534-over','flat10cdrincspinoff','ssp585']#,'1pctco2','ssp245']#,'ssp126','flat10zecincspinoff']#, 'flat10cdrincspinoff','abrupt-4xco2','ssp119','ssp460','ssp370']
N = 200
...
test_files_tas = filter_climate_files(files = potential_files, scenarios = TEST_SCENARIOS, indicators = ['tas'])
...
test_files_with_baseline_tas = [(get_baseline_filename(filename=filename, files= potential_files), filename) for filename in test_files_tas]

# process regional averages etc for tas files
train_data_df_tas = [process_scenarios(experiment_scenario_path = f'{MODEL_PATH}/{experiment}', simulation_name = experiment, baseline_scenario_path = f'{MODEL_PATH}/{baseline}', delete_first_years = 0, monthly_trend = monthly_flag, smoothed = use_smoothing) for (baseline,experiment) in train_files_with_baseline_tas]
test_data_df_tas = [process_scenarios(experiment_scenario_path = f'{MODEL_PATH}/{experiment}', simulation_name = experiment, baseline_scenario_path = f'{MODEL_PATH}/{baseline}', delete_first_years = 0, monthly_trend = monthly_flag, smoothed = use_smoothing) for (baseline,experiment) in test_files_with_baseline_tas]
...
for indicator in INDICATORS:
    if indicator == 'tas': 
        ...
        train_data_df_indicator = [process_scenarios(experiment_scenario_path = f'{MODEL_PATH}/{experiment}', simulation_name = experiment, baseline_scenario_path = f'{MODEL_PATH}/{baseline}', delete_first_years = 0, monthly_trend = monthly_flag, smoothed = use_smoothing) for (baseline,experiment) in train_files_with_baseline_indicator]
        test_data_df_indicator = [process_scenarios(experiment_scenario_path = f'{MODEL_PATH}/{experiment}', simulation_name = experiment, baseline_scenario_path = f'{MODEL_PATH}/{baseline}', delete_first_years = 0, monthly_trend = monthly_flag, smoothed = use_smoothing) for (baseline,experiment) in test_files_with_baseline_indicator]
...
pickle_filename = "/hdrive/all_users/kainverena/formatted_data/test_data.pkl"
